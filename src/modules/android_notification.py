...
class AndroidNotification(Notification):
    ...
    def cancel_notification(self, notification_id=0):
        '''
        Cancel the specified notification.

        :param notification_id: The ID of the notification to cancel.
                                If not specified, defaults to 0, which is
                                the ID used in `notify()` method.

        .. versionadded:: 1.5.0
        '''
        self._get_notification_service().cancel(notification_id)
    ...
    def _build_notification(self, title):
        ...
        # Disable all default notification behaviors (sound, vibration, etc.)
        noti.setDefaults(0)
    ...
    def _set_open_behavior(notification):
        ...
        pending_intent = PendingIntent.getActivity(
            app_context, 0, notification_intent, 
